...
# looks like these files rely on NumPy's deprecated API, so we cannot disable the warning for now
...
cpdef double q_exp(double q, double x):
  cdef double base
  if q == 1.:
    return math.exp(x)
  else:
    base = math.fmax(1. + (1.-q)*x, 0.)
    return base ** (1./(1.-q))
...
cpdef np.ndarray[np.float32_t, ndim=2] _binary_search_perplexity(
    ...
    cdef long n_steps = 500 # large enough to find huge bandwidths under q >> 1 
        ...
        for l in range(n_steps):
            ...
            for j in range(n_neighbors):
                if j != i or using_neighbors:
                    P[i, j] = q_exp(deformation,
                        -sqdistances[i, j] * beta)
                    sum_Pi += P[i, j]
